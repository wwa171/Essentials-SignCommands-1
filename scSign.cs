...
namespace SignCommands {
  public class ScSign {
    ...
    private IEnumerable<List<string>> ParseCommands(string text) {
      ...
      text = text.Replace(SignCommands.config.CommandsStartWith, TShock.Config.Settings.CommandSpecifier);
      ...
      var cmdStrings = text.Split(Convert.ToChar(TShock.Config.Settings.CommandSpecifier));
    ...
    public void ExecuteCommands(ScPlayer sPly) {
      ...
      if (_users.Count > 0 && !_users.Contains(sPly.TsPlayer.Name)) {
        if (sPly.AlertPermissionCooldown == 0) {
      ...
      foreach (var cmdPair in commands) {
        ...
        string log = string.Format("{0} executed: {1}{2} [Via sign command].", 
          sPly.TsPlayer.Name, TShock.Config.Settings.CommandSpecifier, cmdText);
